<template>
  <div :class="['maincontent',{'switch':navChange}]">
    <div class="left">
      <ul class="navList">
        <li v-for="item in listData"><a :href="item.url" target="right"><span><img :src="item.bgImage"></span>{{item.name}}</a></li>
      </ul>
    </div>
    <div class="right">
      <div class="arrow" @click="arrowChange">{{toggleArrow}}</div>
      <router-view class="child-view" name='a'></router-view>
      <!--<iframe src="../page/order" frameborder="0" scrolling="auto" width="100%" height="100%" name="right" ></iframe>-->
    </div>
  </div>
</template>
<style lang="scss">
